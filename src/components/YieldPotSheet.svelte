<script lang="ts">
	...
	// Week progress for draw ring
	const weekProgress = $derived.by(() => {
		const now = new Date();
		return (now.getDay() * 24 * 60 + now.getHours() * 60 + now.getMinutes()) / (7 * 24 * 60);
	});
	const timeLeft = $derived.by(() => {
		const now     = new Date();
		const day     = now.getDay();
		const daysLeft = day === 0 ? 0 : 7 - day - 1;
			...
			sdk.rpc.profile.getProfileByAddress(m.member as `0x${string}`)
				.then(p => {
					...
					const imageUrl = raw?.startsWith('ipfs://')
						...
						: raw ?? undefined;
					memberProfiles = new Map(memberProfiles).set(m.member.toLowerCase(), { name: p.name ?? undefined, imageUrl });
				})
				.catch(() => {});
		}
	}
	...
	>
		...
		<div class="mx-4 mb-4">
			...
			{:else if members.length === 0}
				<p class="py-4 text-center text-sm" style="color:var(--text-dim);">No contributions yet. Be the first!</p>
			{:else}
